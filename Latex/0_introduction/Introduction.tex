...
\begin{itemize}
	\item Lifelong immunity acquired after recovery, R = recovered.
	\item Death of infected individuals, \textcolor{red}{R = removed}.
\end{itemize}
...
After having introduced the basic framework of the model, it is now presented the set of differential equations that describe the dynamic of infection:
\begin{equation}
	\begin{cases}
...
Observing how the system evolve
DESCRIZIONE DEL COMPORTAMENTO, MENO MATEMATICA
MA TENGO LE FIGURE E PRESENTO UN PO' IL COMPORTAMENTO


Consider now the initial state of the system. At the beginning of the disease, considering that there are few infected, the majority of the population is in the susceptible groups, so $s(0) \approx 1$. Furthermore, during the initial days of contagion diffusion, this quantity remains stable. Considering this approximation, we have 
...
VEDI E AGGIUNGI ANCHE \cite{Tizzoni2014}
 ...
 INTEGRA: "An agent-based model (ABM) is a computational model for simulating the actions and interactions of autonomous agents (both individual or collective entities such as organizations or groups) in order to understand the behavior of a system and what governs its outcomes."
