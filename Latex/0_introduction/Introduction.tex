...
The first approach involves observing and simulating how connected individuals' states are linked to specific behaviors and how this influences the epidemic. This category includes agent-based models. Additionally, the reverse relationship, where disease spread alters individual behavior, has also been considered, as seen in \cite{Granell_2014}.

There is also a broad class of mean-field models that explicitly consider the effects of behavior. In these models, time-varying or state-varying parameters are used, resulting in a non-linear system of equations where the parameters are not constant but change based on information such as disease prevalence \cite{CITA}.

Another possibility involves modifying the structure or connections in the network used to simulate disease evolution \cite{CITA}. In network-based models, data extracted from social network structures \cite{CITA} or small-world models \cite{CITA} are often used to simulate connections between people more realistically.
...
\begin{figure}[h]
	...
	\caption[Metacritical effect]{Effect of awarennes communication impact on the onset of the epidemic Pictures taken from \cite{Granell2013, Granell_2014}. DA FINIRE DI SCRIVERE.}
...
In the probabilistic framework, another area involves the use of game-theory principles. These are used to explore strategic interactions between individuals, where participants act to maximize their utility, potentially influencing the actions of others. The concept of Nash Equilibrium is also important in this context. It is defened as " a set of strategies such that no player has an incentive to unilaterally deviate from the present strategy" \cite{Wang_2015_review}. That is, the Nash Equilibrium leads individuals to adopt strategies consistent with their goal of maximizing their own benefit or utility in a perfectly rational way, forming best responses to one another.
%% vecchio sotto



In the probabilistic framework, another area included involves the use of game-theory principles. The same "rational agent" assumption found in threshold models is used here, requiring each agent to have its own cost or utility function, with individual actions based on maximizing personal payoff \cite{Auld_2003}. Many articles use this idea to model how populations vary their behavior during an epidemic. One such example is \cite{Nunner2021}, where different utility functions are defined to model the trade-off between social well-being gained from maintaining connections, the fatigue of maintaining them, and the physical harm those connections may cause.

The observed results show that when people's behavior is based on maximizing their net benefit, individual risk perception plays a significant role in shaping the cost function. Models derived using this so-called co-evolutionary approach exhibit a highly correlated dynamic between the two layers. There is a feedback loop between infection spread and behavioral adaptation, and structural modifications in the network also occur in the simulated scenarios.
