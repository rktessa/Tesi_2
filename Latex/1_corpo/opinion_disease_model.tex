...
\begin{itemize}
	...
	\item  $\mathcal{R}(t)$ is the reprodutction number, derived as in the $SIR$ model as the ratio between the $\beta$, and $\gamma$ coefficients.
...
These ranges are chosen based on the assumption that the "fatigue" rate realistically spans between two and forty days, a range supported by prior research, such as the study in \cite{trova e cita world in data}. For the behavior conversion rate, both low and high values for transition rates are included. We observe the dynamics across all states, recording key metrics for each simulation, such as the final compartment values, peak values, and the time of peak occurrence. Additionally, for generating the sensitivity plots, the reproductive rates derived from the coefficient combinations in equation \eqref{eq:behave_rate} are applied. 
...
\begin{figure}[h]
	\centering
	\subfloat[][\emph{Max Against compartment}]
	{\includegraphics[width=.38\textwidth]{1_corpo/figure/behavioural_equilibrium/max_againste_heatmap_1}} \quad
	\subfloat[][\emph{Against evolution in simulation}]
	{\includegraphics[width=.58\textwidth]{1_corpo/figure/behavioural_equilibrium/against_evolution_vari_1}} \\
...
\begin{figure}[h]
	\centering
	\subfloat[][\emph{Max Against compartment}]
	{\includegraphics[width=.38\textwidth]{1_corpo/figure/behavioural_equilibrium/max_againste_heatmap_2}} \quad
	\subfloat[][\emph{Against evolution in simulation}]
	{\includegraphics[width=.58\textwidth]{1_corpo/figure/behavioural_equilibrium/against_evolution_vari_2}} \\
		\caption[Max against first case]{a) The max value of Against varying the $\lambda_1, \lambda_2$. b) The evolution of several Against compartment dynamic, fixing $k_1, \lambda_1$, and $k_2$ and varying only the $\lambda_2$.}
