...
To simplify the comprehension of the unerline dynamic of the system, facendo un analogia con quello che in epidemiologia viene chiamato "reproduction number",  è possibile trovare una relazione osservando le equazionni del sistema \ref{eq:behavioural_eq}, chthat will be called "behavior conversion rate". It is the result of the ratio between the persuasion rate and the fatigue decay rate. It can be expressed as a measure of transmission potential of the social contagion.
It is found that depends on the comparison between the ratios that can be calculated with the formula: 
\begin{equation}
	\mathcal{B}_i =\frac{ k_i }{\lambda_i}
	\label{eq:behave_rate}
\end{equation}
This expression represent how can be calculated the Behavioral conversion number related to possible course of action that can be adopted. So, with $\mathcal{B}_1$ we repreente the behavior conversion associated with compliant compartment, and with $\mathcal{B}_2$, the one related to against.


The behaviour with the grater value has a dominant effect on the final stable value reached by the compartments.



====================================================
DA SPOSTARE DOPO PRESENTAZIONE COMPORTAMENTO MODELLO, FATTA OSSERVANDO LE SIMULAZIONI
...
These effects can be explained looking at the equilibrium for time that goes to infinite. 
