...
SC_0 = 0; SA_0 = 0; SH_0 = 1; RC_0 = 0; RA_0 = 0; IC_0 =0; IA_0 = 0;    
psi_vec = linspace(1,3,6); d1 = length(psi_vec);
E_0_vec = zeros(d1); B1_vec = zeros(d1);R0_vec = zeros(d1);
...
for i = 1: d1
    ...
    plot(time_epi,x_sc, 'linewidth',2 ,'DisplayName',txt)
    title("Susceptibles ")
    xlabel("t[days]");
    ylabel("SC[t]");
end 
...
for i = 2:d1 
    ...
    [fig2,x_sh, x_sc, x_sa,y_c, y_a,z_rc,z_ra,time_epi] = epi_behaviour(fig2,beta,gamma,delta_v,rho_v,psi_vec(i),k_1,k_2,k_1,k_2,k_1,k_2,lambda_1,lambda_2,lambda_1,lambda_2,lambda_1,lambda_2,epsilon_v,phi_n,SC_0,SA_0,SH_0, time, E_0_vec(i),title_fig);
    txt = "\psi ="+num2str(psi_vec(i)); 
    plot(time_epi, y_c+y_a,'linewidth',2,'DisplayName',txt)
...
function [lambda_1,lambda_2,k1,k2,time,title_fig, beta,gamma,delta,rho,epsilon,psi,phi_n] = scenario(caso)
    ...
    if caso == 0
        ...
        time = 3000;
        title_fig = 'epi_behav_sim_psi.pdf';
    end
    if caso == 1
        B1 = 0.89;
        B2 = 0.45;
...
function [fig,x_sh, x_sc, x_sa,y_c, y_a,z_rc,z_ra,time_epi]= epi_behaviour(fig,beta,gamma,delta,rho,psi,k1,k2,k3,k4,k5,k6,lambda1,lambda2,lambda3,lambda4,lambda5,lambda6,epsilon,phi_n,C_zero,A_zero,H_zero, time, E_0,title_fig)
